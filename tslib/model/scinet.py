...
class SCIBlock(nn.Module):
    def __init__(self, d_model, kernel_size=5, dropout=0.0, causal_block_level = 4):
        super(SCIBlock, self).__init__()
        self.splitting = Splitting()
        self.modules_even, self.modules_odd, self.interactor_even, self.interactor_odd = [CausalConvBlock(d_model) for _ in range(causal_block_level)]
...
class SCINet(nn.Module):
    def __init__(self, d_model, current_level=3, kernel_size=5, dropout=0.0, causal_block_level = 4):
        super(SCINet, self).__init__()
        
        self.current_level = current_level
        self.working_block = SCIBlock(d_model, kernel_size, dropout, causal_block_level)
        if current_level != 0:
...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        if self.num_stacks == 1:
            self.sci_net_1 = SCINet(configs.enc_in, current_level = configs.current_level, dropout=configs.dropout, causal_block_level = configs.causal_block_level)
            ...
            self.sci_net_1, self.sci_net_2 = [SCINet(configs.enc_in, current_level = configs.current_level, 
                                                     dropout=configs.dropout, causal_block_level = configs.causal_block_level) for _ in range(2)]
            self.projection_1 = nn.Conv1d(self.seq_len, self.pred_len, kernel_size=1, stride=1, bias=False)
            # self.projection_2 = nn.Conv1d(self.seq_len+self.pred_len, self.seq_len+self.pred_len,
            #                                     kernel_size = 1, bias = False)
            self.projection_2 = nn.Conv1d(self.seq_len+self.pred_len, self.pred_len, kernel_size = 1, bias = False)

        if self.task_name == 'regression':
            self.flatten = nn.Flatten(start_dim=-2)
            self.dropout = nn.Dropout(configs.dropout)
            self.projection = nn.Linear(
                configs.pred_len if self.num_stacks == 1 else self.seq_len + self.pred_len, 
                configs.num_class
            )
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec, mask=None):
        if self.task_name == 'long_term_forecast' or self.task_name == 'short_term_forecast':
            ...
            return dec_out
        if self.task_name == 'regression':
            dec_out = self.regression(x_enc, x_mark_enc, x_dec, x_mark_dec)  # [B,pred_len,C]
            return dec_out
    ...
    def get_position_encoding(self, x):
        ...
        scaled_time = position.unsqueeze(1) * self.inv_timescales.unsqueeze(0).to(x.device)  # 5 256
        signal = torch.cat([torch.sin(scaled_time), torch.cos(scaled_time)], dim=1).to(x.device)  # [T, C]
        signal = F.pad(signal, (0, 0, 0, self.pe_hidden_size % 2)).to(x.device)
