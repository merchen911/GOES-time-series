...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        if self.task_name == 'classficiation':
            embedding_mode = True
        
        self.decomp = DECOMP(self.ma_type, alpha, beta)
        self.net = Network(seq_len, pred_len, patch_len, stride, padding_patch, embedding_mode = embedding_mode)
...
#         self.decomp = DECOMP(self.ma_type, alpha, beta)
#         self.net = Network(seq_len, pred_len, patch_len, stride, padding_patch, embedding_mode)
#         # self.net_mlp = NetworkMLP(seq_len, pred_len) # For ablation study with MLP-only stream
#         # self.net_cnn = NetworkCNN(seq_len, pred_len, patch_len, stride, padding_patch) # For ablation study with CNN-only stream
